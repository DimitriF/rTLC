...
#   }
  ...
  output$TableDimension <-renderTable({
    ...
    if(!is.null(inFile)){
      data.saved <- as.data.frame(read_excel(as.character(inFile[1,4]),sheet=1))
      validate(
...
#   label.color <- gsub(1,'red',gsub(2,'green',gsub(3,'blue',gsub(4,'gray',label.color))))
  ...
  data<- data$scores[,1:10]#ChemometricsWithR::scores(data,npc=10)
...
output$pca.plot.pair <- renderPlot({
  data = data$scores[,1:input$PCA.pair]##ChemometricsWithR::scores(model.pca(),npc=input$PCA.pair)
...
output$pca.table.1<-renderDataTable({
  data<-model.pca()
  data<-data$scores[,1:4]#ChemometricsWithR::scores(data,npc=4)
...
Moutlier.pca.1<-reactive({
  ...
  Moutlier(data$scores[,as.numeric(input$comp.outlier.pca.1)],quantile = input$quantile.outlier.pca.1, plot=F)
  })
...
output$quantile.outlier.pca.table<-renderDataTable({
  truc = Moutlier(data$scores[,as.numeric(input$comp.outlier.pca.1)],quantile = input$quantile.outlier.pca.1)
